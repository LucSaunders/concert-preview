...
export function fetchEvents(query = 'Raleigh') {
  const request = axios.get(
    `${EVENT_ROOT_URL}&q=music&location=${query}&t=This+Week${EVENT_API_KEY}`
  );
...
export function fetchVideos(query) {
  const request = axios.get(
    `${VIDEO_ROOT_URL}&part=snippet&q=${query}&type=video&maxResults=4${VIDEO_API_KEY}`
  );
